<template>
    <div class="container">
      ...
      <div class="main-add-navi">
        ...
        <router-link :to="{ name: 'EditRecipient'}"><button id="product" @click="edit()" type="submit">Edit Recipient</button></router-link>
...
export default {
    ...
    mounted(){
      var app = this
      this.axios.all([
        this.axios.get('recipient/' + this.$route.params.id),
      ]).then( this.axios.spread((recipient) => {
        console.log(recipient)
        app.recipient = recipient.data.payload
      })).catch(error => {
        console.log(error)
      })
    },
    methods:{
      edit(){
        ...
        this.axios.patch('recipient/' + this.$route.params.id, updateData).then( res => {
